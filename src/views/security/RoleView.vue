      ...
      <el-table :data="filteredData" style="width: 100%" v-loading="loading">
        ...
        <el-table-column label="类型" width="150">
          <template v-slot="scope">
            <el-tag type="warning" round>{{ scope.row.type || 'Namespaced' }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column label="权限规则" min-width="400">
          <template v-slot="scope">
...
const filteredData = computed(() => {
  ...
  return roles.value.filter(item => {
    ...
    return nameMatch || namespaceMatch || rulesMatch
...
const getRoleData = async () => {
  try {
    ...
    roles.value = [
      {
        ...
        rules: [
          ...
          { verbs: ['get', 'list'], apiGroups: [''], resources: ['pods/log'] }
        ],
      ...
      },
      {
        ...
        type: 'Namespaced',
        rules: [
          { verbs: ['create', 'update', 'patch', 'delete'], apiGroups: ['apps'], resources: ['deployments'] },
    ...
    console.error('获取 Role 数据失败:', error)
